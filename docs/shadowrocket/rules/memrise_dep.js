...
// Memrise = type=http-response,pattern=^https:\/\/api\.memrise\.com\/v1\.20\/me\/paths\/.+,requires-body=1,script-path=https://raw.githubusercontent.com/KikMyaz/kikmyaz.github.io/dev/docs/shadowrocket/rules/memrise.js
// api.memrise.com

/*
let url = $request.url;
const cons = "current_status/";

if(url.endsWith(cons)) {
    var obj = JSON.parse($response.body);
    obj.user_scenario.is_premium = true;
    $done({body: JSON.stringify(obj)});
} else {
    var obj = $response.body;
    obj = obj.replace(/\"is_locked\":true/g,'\"is_locked\":false');
    obj = obj.replace(/\"is_premium\":true/g,'\"is_premium\":false');
    console.log(obj);
    $done({ obj });
}
*/
